...
int main(int argc, char *argv[]) {
  ...
  double mem_term_kib = 0, mem_kill_kib = 0, swap_term_kib = 0,
         swap_kill_kib = 0;
  ...
  while ((c = getopt_long(argc, argv, short_opt, long_opt, NULL)) != -1) {
    ...
    case 'N':
      ...
      fprintf(
          stderr,
          "Notifying through D-Bus, argument '%s' ignored for compatability\n",
          optarg);
      break;
    case 'd':
      enable_debug = 1;
    ...
    case 'h':
      fprintf(
          ...
          "Usage: %s [OPTION]...\n"
          "\n"
          "  -m PERCENT[,KILL_PERCENT] set available memory minimum to PERCENT "
          "of total\n"
          "                            (default 10 %%).\n"
          "                            earlyoom sends SIGTERM once below "
          "PERCENT, then\n"
          "                            SIGKILL once below KILL_PERCENT "
          "(default PERCENT/2).\n"
          "  -s PERCENT[,KILL_PERCENT] set free swap minimum to PERCENT of "
          "total (default\n"
          "                            10 %%).\n"
          "                            Note: both memory and swap must be "
          "below minimum for\n"
          "                            earlyoom to act.\n"
          "  -M SIZE[,KILL_SIZE]       set available memory minimum to SIZE "
          "KiB\n"
          "  -S SIZE[,KILL_SIZE]       set free swap minimum to SIZE KiB\n"
          "  -i                        user-space oom killer should ignore "
          "positive\n"
          "                            oom_score_adj values\n"
          "  -n                        enable d-bus notifications\n"
          "  -d                        enable debugging messages\n"
          "  -v                        print version information and exit\n"
          "  -r INTERVAL               memory report interval in seconds "
          "(default 1), set\n"
          "                            to 0 to disable completely\n"
          "  -p                        set niceness of earlyoom to -20 and "
          "oom_score_adj to\n"
          "                            -100\n"
          "  --prefer REGEX            prefer to kill processes matching "
          "REGEX\n"
  ...
  if (prefer_cmds) {
    ...
    if (regcomp(args.prefer_regex, prefer_cmds, REG_EXTENDED | REG_NOSUB) !=
        0) {
  ...
  fprintf(stderr,
          "sending SIGTERM when mem <= " PRIPCT " and swap <= " PRIPCT ",\n",
          args.mem_term_percent, args.swap_term_percent);
  fprintf(stderr,
          "        SIGKILL when mem <= " PRIPCT " and swap <= " PRIPCT "\n",
...
/* Calculate the time we should sleep based upon how far away from the memory
 * and swap limits we are (headroom). Returns a millisecond value between 100
 * and 1000 (inclusive). The idea is simple: if memory and swap can only fill up
 * so fast, we know how long we can sleep without risking to miss a low memory
 * event.
 */
static unsigned sleep_time_ms(const poll_loop_args_t *args,
                              const meminfo_t *m) {
  // Maximum expected memory/swap fill rate. In kiB per millisecond ==~ MiB per
  // second.
  const long long mem_fill_rate =
      6000; // 6000MiB/s seen with "stress -m 4 --vm-bytes 4G"
  ...
  long long mem_headroom_kib =
      (long long)((m->MemAvailablePercent - args->mem_term_percent) * 10 *
                  (double)m->MemTotalMiB);
  if (mem_headroom_kib < 0) {
    mem_headroom_kib = 0;
  }
  long long swap_headroom_kib =
      (long long)((m->SwapFreePercent - args->swap_term_percent) * 10 *
                  (double)m->SwapTotalMiB);
  if (swap_headroom_kib < 0) {
    swap_headroom_kib = 0;
  }
  long long ms =
      mem_headroom_kib / mem_fill_rate + swap_headroom_kib / swap_fill_rate;
  if (ms < min_sleep) {
    return min_sleep;
  }
  if (ms > max_sleep) {
...
static void poll_loop(const poll_loop_args_t *args) {
  ...
  while (1) {
    ...
    if (m.MemAvailablePercent <= args->mem_kill_percent &&
      ...
      warn("low memory! at or below SIGKILL limits: mem " PRIPCT
           ", swap " PRIPCT "\n",
           args->mem_kill_percent, args->swap_kill_percent);
      sig = SIGKILL;
      ...
      print_mem_stats(warn, m);
      warn("low memory! at or below SIGTERM limits: mem " PRIPCT
           ", swap " PRIPCT "\n",
           args->mem_term_percent, args->swap_term_percent);
      sig = SIGTERM;
    }
