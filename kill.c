...
static void notify(const char *summary, const char *body) {
  ...
  // 'string:summary text' 'string:and body text'
  execl("/usr/bin/dbus-send", "dbus-send", "--system", "/",
        "net.nuetzlich.SystemNotifications.Notify", summary2, body2, NULL);
...
void kill_largest_process(const poll_loop_args_t *args, int sig) {
  ...
  while (1) {
    ...
    if ((args->prefer_regex || args->avoid_regex)) {
      ...
      if (args->prefer_regex &&
          regexec(args->prefer_regex, cur.name, (size_t)0, NULL, 0) == 0) {
        cur.badness += BADNESS_PREFER;
      }
      if (args->avoid_regex &&
          regexec(args->avoid_regex, cur.name, (size_t)0, NULL, 0) == 0) {
  ...
  if (candidates <= 1 && victim.pid == getpid()) {
    warn("Only found myself (pid %d) in /proc. Do you use hidpid? See "
  ...
  if (sig != 0 || enable_debug) {
    warn("sending %s to process %d uid %d \"%s\": badness %d, VmRSS %lld MiB\n",
         sig_name, victim.pid, victim.uid, victim.name, victim.badness,
  ...
  // Send the GUI notification AFTER killing a process. This makes it more
  // likely that there is enough memory to spawn the notification helper.
  if (sig != 0) {
