...
// An error is indicated by storing an error message in tuple->err and returning
// 0.
static double parse_part(term_kill_tuple_t *tuple, const char *part,
                         long long upper_limit) {
  errno = 0;
  char *endptr = 0;
  double val = strtod(part, &endptr);
  if (*endptr != '\0') {
    snprintf(tuple->err, sizeof(tuple->err), "trailing garbage '%s'", endptr);
    return 0;
  }
  if (errno) {
    snprintf(tuple->err, sizeof(tuple->err), "converson error: %s",
             strerror(errno));
    return 0;
  }
  if (val > (double)upper_limit) {
    snprintf(tuple->err, sizeof(tuple->err), "value %lf exceeds limit %lld",
             val, upper_limit);
    return 0;
  }
  if (val < 0) {
    snprintf(tuple->err, sizeof(tuple->err), "value %lf below zero", val);
    return 0;
  }
  return val;
}
...
term_kill_tuple_t parse_term_kill_tuple(const char *optarg,
                                        long long upper_limit) {
  ...
  if (strlen(optarg) > (sizeof(buf) - 1)) {
    snprintf(tuple.err, sizeof(tuple.err), "argument too long (%zu bytes)",
             strlen(optarg));
    return tuple;
  }
  ...
  if (tuple.term < tuple.kill) {
    warn("warning: SIGTERM value %.2lf is below SIGKILL value %.2lf, setting "
         "SIGTERM = SIGKILL = %.2lf\n",
